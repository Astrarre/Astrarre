...
import io.github.astrarre.v0.api.rendering.gui.GuiGraphics;
...
public class GuiScreen extends HandledScreen<GuiScreenHandler> {
	...
	protected void fillGradient2(GuiGraphics graphics) {
		...
		try (GuiGraphics.Closeable _temp3 = graphics.setOffsetCloseable(100, 100)) {
			try (GuiGraphics.Closeable _temp2 = graphics.rotate(50, 50, test)) {
				try (GuiGraphics.Closeable _temp = graphics.rotate(0, 0, test)) {
