...
public class HopperBlockEntityTransformer_InventoryCrossCompatibilityHooks {
	private static final Supplier<WorldFunction<Participant<ItemKey>>> FUNCTION = FabricParticipants.ITEM_WORLD.getExcluding(Collections.singleton(TransferInternalAstrarre.HOPPER_BLOCK_ENTITY_GET_INVENTORY));
	...
	public static Inventory get(BlockPos posM, World worldM, BlockState stateM, BlockEntity entityM) {
		...
		Participant up = function.get(Direction.UP, stateM, worldM, posM, entityM),
				...
				north = function.get(Direction.NORTH, stateM, worldM, posM, entityM),
				south = function.get(Direction.SOUTH, stateM, worldM, posM, entityM),
				west = function.get(Direction.WEST, stateM, worldM, posM, entityM),
				east = function.get(Direction.EAST, stateM, worldM, posM, entityM);
