...
public class TestModBlock extends Block implements BlockEntityProvider, InventoryProvider {
	...
	public SidedInventory getInventory(BlockState state, WorldAccess world, BlockPos pos) {
		if(world instanceof World) {
			return TransferInternalAstrarre.getSidedInventoryAt((io.github.astrarre.v0.world.World) world, (io.github.astrarre.v0.util.math.BlockPos) pos);
		}
	...
	public static class Entity extends BlockEntity implements io.github.astrarre.access.v0.api.provider.BlockEntityProvider, Participant<ItemKey> {
		...
		public int extract(@Nullable Transaction transaction, ItemKey type, int quantity) {
			...
			view.compute(nbt, (k, i) -> i - quantity);
			return toExtract;
		}

		@Override
		public int insert(@Nullable Transaction transaction, ItemKey type, int quantity) {
			this.storage.get(transaction).computeIfAbsent(type.asItem(), t -> new DiffKey.Map<>()).get(transaction).compute(type.getTag(), (k, i) -> (i == null ? 0 : i) + quantity);
			return quantity;
		}
