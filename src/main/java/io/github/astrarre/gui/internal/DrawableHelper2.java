...
public class DrawableHelper2 {
	 ...
	 */
	public static Iterable<Vector3f[]> iterateAsQuads(Vector3f[] vertices) {
		return () -> new Iterator<Vector3f[]>() {
			final Vector3f first = vertices.length > 0 ? vertices[0] : null;
			int index = 1;

			@Override
			public boolean hasNext() {
				return this.index < vertices.length;
			}

			@Override
			public Vector3f[] next() {
				Vector3f[] quads = new Vector3f[4];
				quads[0] = this.first;
				quads[1] = vertices[this.index++];
				quads[2] = vertices[this.index++];
				if (this.index == vertices.length) {
					quads[3] = vertices[2];
				} else {
					quads[3] = vertices[this.index];
				}
				return quads;
			}
		};
