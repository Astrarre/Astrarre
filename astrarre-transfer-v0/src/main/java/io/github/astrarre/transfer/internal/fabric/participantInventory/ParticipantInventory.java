package io.github.astrarre.transfer.internal.fabric.participantInventory;

import java.util.Set;

import io.github.astrarre.itemview.v0.fabric.TaggedItem;
import io.github.astrarre.transfer.internal.fabric.access.ItemStackAccess;
...
public class ParticipantInventory implements Inventory, FilteringInventory {
	...
	public ItemStack getStack(int slot) {
		...
		if (slot == BUFFER_EXTRACTION_SLOT) {
			try (Transaction transaction = new Transaction(false)) {
				...
				final TaggedItem item = this.slot.getType(transaction);
	...
	public void setStack(int slot, ItemStack stack) {
		...
		TaggedItem item = TaggedItem.of(stack);
		this.watchStack(item, stack);
	 ...
	 */
	private void watchStack(TaggedItem item, ItemStack stack) {
