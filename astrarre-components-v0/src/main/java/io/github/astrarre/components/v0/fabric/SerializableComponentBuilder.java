...
public abstract class SerializableComponentBuilder<C, V, T extends Component<C, V>> extends SynchronizableComponentBuilder<C, V, T> {
	protected final Map<String, Pair<Component<C, ?>, FabricSerializer<?, ?>>> serializeInternal;
	 ...
	 */
	public SerializableComponentBuilder<C, V, T> serialize(FabricSerializer<V, ?> serializer) {
		if (this.set) {
